...
export function useScanConfigDb() {
    return {
        ...
        getScanConfigList: async (projectId?: number) => {
            let sql = "select id, name, exclude_patterns excludePatterns, include_patterns includePatterns, exclude_system_dirs excludeSystemDirs, exclude_hidden_dirs excludeHiddenDirs, is_global isGlobal, project_id projectId, create_time createTime from t_scan_config";
        ...
        getScanConfigById: async (id: number) => {
            const sql = "select id, name, exclude_patterns excludePatterns, include_patterns includePatterns, exclude_system_dirs excludeSystemDirs, exclude_hidden_dirs excludeHiddenDirs, is_global isGlobal, project_id projectId, create_time createTime from t_scan_config where id = $1";
        ...
        getDefaultScanConfig: async () => {
            const sql = "select id, name, exclude_patterns excludePatterns, include_patterns includePatterns, exclude_system_dirs excludeSystemDirs, exclude_hidden_dirs excludeHiddenDirs, is_global isGlobal, project_id projectId, create_time createTime from t_scan_config where is_global = 1 order by id asc limit 1";
            let dataResult: DataResultType<RowScanConfigType> = { code: 0, msg: "", data: {} as RowScanConfigType };
            try {
        ...
        upsertScanConfig: async (data: RowScanConfigType) => {
            ...
            try {
                if (data.id && data.id > 0) {
                    const sql = "UPDATE t_scan_config SET name = $1, exclude_patterns = $2, include_patterns = $3, exclude_system_dirs = $4, exclude_hidden_dirs = $5, is_global = $6, project_id = $7 WHERE id = $8";
                    const r = await (await db()).execute(sql, [
                        data.name, data.excludePatterns, data.includePatterns,
                        data.excludeSystemDirs, data.excludeHiddenDirs, data.isGlobal, data.projectId ?? null, data.id,
                    ]);
                    if (r.rowsAffected > 0) { dataResult.code = 0; dataResult.data = data.id; dataResult.msg = "更新扫描规则成功"; }
                    else { dataResult.msg = "更新扫描规则失败"; }
                } else {
                    const sql = "INSERT INTO t_scan_config (name, exclude_patterns, include_patterns, exclude_system_dirs, exclude_hidden_dirs, is_global, project_id, create_time) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id;";
                    const r = await (await db()).execute(sql, [
                        data.name, data.excludePatterns, data.includePatterns,
